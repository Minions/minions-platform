...
import type { MinionSpec } from '../domain/MinionSpec';
import type { MinionMessage } from '../domain/MinionMessage';
...
export interface IMinion {
  ...
  /**
   * Workbench containing shared contextual knowledge
   *
   * If the minion was spawned with a workbench, this provides access to:
   * - Files relevant to the current task
   * - Project facts discovered during work
   *
   * The workbench reference is readonly (doesn't change), but its contents
   * can be modified (files added, facts added).
   *
   * If the minion was spawned without a workbench, this is undefined.
   *
   * Note: Typed as IWorkbench from @minions/domain-types (the shared
   * workbench contract) rather than the conductor-specific implementation,
   * to avoid a circular dependency with the conductor library.
   */
